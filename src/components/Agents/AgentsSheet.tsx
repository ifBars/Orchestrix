...
export function AgentsSheet({ onClose }: AgentsSheetProps) {
  const [
    agentPresets,
        ...
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Human-authored agent presets from <code className="rounded bg-muted px-1 py-0.5">.agents/agents</code>,
              <code className="ml-1 rounded bg-muted px-1 py-0.5">.agent/agents</code>,
              and <code className="ml-1 rounded bg-muted px-1 py-0.5">.opencode/agents</code>.
            </p>
          </div>
          <Button variant="ghost" size="icon" className="h-8 w-8" onClick={onClose} title="Return to chat">
            <X size={14} />
          </Button>
        </div>

        <div className="grid min-h-0 flex-1 gap-4 lg:grid-cols-[320px_1fr]">
          <section className="min-h-0 rounded-xl border border-border bg-card/60 p-3">
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="mt-1 flex items-center gap-2 text-[10px] text-muted-foreground">
                          ...
                          {preset.validation_issues.length > 0 && (
          ...
          <section className="min-h-0 rounded-xl border border-border bg-card/60 p-4">
            <form className="flex h-full min-h-0 flex-col gap-3" onSubmit={savePreset}>
                ...
                <div className="flex flex-wrap gap-4 text-xs">
                  <label className="inline-flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) => setForm((prev) => ({ ...prev, toolWrite: e.target.checked }))}
                    />
                    write
                  </label>
                  <label className="inline-flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) => setForm((prev) => ({ ...prev, toolEdit: e.target.checked }))}
                    />
                    edit
                  </label>
                  <label className="inline-flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) => setForm((prev) => ({ ...prev, toolBash: e.target.checked }))}
                    />
                    bash
                  </label>
                </div>
