# Building code-chunk: AST Aware Code Chunking | Shoubhit Dash
...
## [The Problem with Text Splitters](#the-problem-with-text-splitters)
...
## [How code-chunk Works](#how-code-chunk-works)

### [Step 1: Parse with tree-sitter](#step-1-parse-with-tree-sitter)
...
From a filepath, we detect the language and load the appropriate tree-sitter grammar. The result is a structured tree where every node has a type (function\_declaration, class\_body, import\_statement, etc.) and byte ranges.

### [Step 2: Extract Entities](#step-2-extract-entities)
...
### [Step 3: Build the Scope Tree](#step-3-build-the-scope-tree)
...
### [Step 4: The Chunking Algorithm](#step-4-the-chunking-algorithm)
...
### [Step 5: Contextualized Text](#step-5-contextualized-text)
...
## [What We Added Beyond the Paper](#what-we-added-beyond-the-paper)
...
### [The Benchmark Was Too Easy](#the-benchmark-was-too-easy)
...
### [Fixing the Eval](#fixing-the-eval)
...
+---------------------+----------+-------+ | Chunker | Recall@5 | IoU@5 | +---------------------+----------+-------+ | code-chunk (AST) | 70.1% | 0.43 | | chonkie-code | 49.0% | 0.38 | | Fixed-size baseline | 42.4% | 0.34 | +---------------------+----------+-------+
...
+----------------------+----------+--------+-------+------------+ | | Duration | Tokens | Cost | Tool Calls | +----------------------+----------+--------+-------+------------+ | Without search | 2.0m | 4.3k | $0.25 | 19 | | With semantic search | 1.2m | 2.4k | $0.20 | 12 | +----------------------+----------+--------+-------+------------+
...
## [Using code-chunk](#using-code-chunk)
